...
gemai.register_action("aurum_mobs:find_prey", function(self)
	for _,object in b.t.ro_ipairs(minetest.get_objects_inside_radius(self.entity.object:get_pos(), aurum.mobs.SEARCH_RADIUS)) do
		if object:get_luaentity() and object:get_luaentity()._aurum_mobs_id and object:get_luaentity().name ~= self.entity.name and aurum.mobs.helper_can_see(self, object) and ((#self.data.hunt_prey == 0) or aurum.match_item_list(object:get_luaentity().name, self.data.hunt_prey)) then

...
function aurum.tools.register_variant(name)
	...
	minetest.register_tool(":" .. properties._enchanted, b.t.combine({_doc_items_create_entry = false}, def, properties, {
		...
		inventory_image = (def.inventory_image and #def.inventory_image > 0 and not def._no_enchanted_inventory_image) and (def.inventory_image .. "^aurum_tools_enchanted.png") or nil,
		wield_image = (def.wield_image and #def.wield_image > 0 and not def._no_enchanted_inventory_image) and (def.wield_image .. "^aurum_tools_enchanted.png") or nil,
	}))
